...
export function validateSessionMemoryActions(actions, facts) {
  ...
  for (let cycle = 0; cycle < SESSION_CYCLES; cycle += 1) {
    ...
    requireLaneAction(cycleActions[0], { role: 'link', label: 'Convert', kind: 'press', timeoutMs: 2000, eventType: 'click', prior: '/convert', axPrior: { kind: 'element_present' }, completion: { kind: 'attribute_equals', role: 'AXGroup', label: 'Convert', attribute: 'AXEnabled', value: true } }, offset + 1);
    ...
    requireLaneAction(cycleActions[33], { role: 'link', label: 'Extract', kind: 'press', timeoutMs: 2000, eventType: 'click', prior: '/convert', axPrior: { kind: 'element_present' }, completion: { kind: 'attribute_equals', role: 'AXGroup', label: 'Extract', attribute: 'AXEnabled', value: true } }, offset + 34);
    requireLaneAction(cycleActions[34], { role: 'link', label: 'Convert', kind: 'press', timeoutMs: 2000, eventType: 'click', prior: '/extract', axPrior: { kind: 'element_present' }, completion: { kind: 'attribute_equals', role: 'AXGroup', label: 'Convert', attribute: 'AXEnabled', value: true } }, offset + 35);
  }
  return actions;
}
...
const jxaPidProcess = expectedPid => `const processMatches=se.processes.whose({unixId:${expectedPid}})();const app=processMatches.length===1?processMatches[0]:null;`;
