...
type EligibilityState =
  | { status: "checking"; revision: string }
  | { status: "available"; revision: string }
  | { status: "unavailable"; revision: string; reason: string | null }
...
export default function SettingsPreview({ request, imageSettings, videoSettings, blockedReason = null }: { request: Settings; imageSettings?: ImageSettingsCapabilities | null; videoSettings?: Pick<VideoSettingsCapabilities, "preview_unavailable_reason"> | null; blockedReason?: string | null }) {
  const descriptorSupported = !request.video_options && (!request.image_options || (imageSettings?.available && (
  ...
  const [eligibility, setEligibility] = useState<EligibilityState>({status:"checking",revision});
  const [eligibilityAttempt, setEligibilityAttempt] = useState(0);
  const currentEligibility = eligibility.revision === revision ? eligibility : {status:"checking",revision} satisfies EligibilityState;
  const supported = currentEligibility.status === "available" && descriptorSupported && !blockedReason;
  const descriptorReason = request.video_options
    ? videoSettings?.preview_unavailable_reason ?? "Explicit video settings previews are unavailable."
    : descriptorSupported ? null : imageSettings?.preview_unavailable_reason ?? "Image settings preview is unavailable.";
  const unavailableReason = blockedReason ?? (currentEligibility.status === "unavailable"
    ? currentEligibility.reason ?? descriptorReason ?? "Preview is unavailable for these settings."
    : currentEligibility.status === "available" ? descriptorReason : null);
  const eligibilityError = currentEligibility.status === "error" ? currentEligibility.message : null;
  const isHeic = /\.(?:heic|heif)$/i.test(request.input_path);
  // Session identity is deliberately independent of eligibility state. A
  // quality edit triggers a fresh check but must not release the PREV-02
  // session or pinned comparison for the same HEIC source.
  const heicSourceKey = isHeic && request.target === "jpeg" && request.image_options && descriptorSupported ? request.input_path : null;
  const active = useRef<string | null>(null);
  const eligibilityActive = useRef<string | null>(null);
  ...
  useEffect(() => {
    if (blockedReason) {
      setEligibility({status:"checking",revision});
      return;
    }
    let current = true;
    setEligibility({status:"checking",revision});
    const eligibilityRequest: PreviewRequest = {
      ...requestRef.current,
      request_id: crypto.randomUUID(),
      source_revision: revision,
    };
    eligibilityActive.current = eligibilityRequest.request_id;
    void api.preview.eligibility(eligibilityRequest).then(
      (answer) => {
        if (!current) return;
        if (eligibilityActive.current === eligibilityRequest.request_id) eligibilityActive.current = null;
        setEligibility(answer.available
          ? {status:"available",revision}
          : {status:"unavailable",revision,reason:answer.reason});
      },
      (cause) => {
        if (!current) return;
        if (eligibilityActive.current === eligibilityRequest.request_id) eligibilityActive.current = null;
        setEligibility({status:"error",revision,message:formatError(cause)});
      },
    );
    return () => {
      current = false;
      if (eligibilityActive.current === eligibilityRequest.request_id) {
        eligibilityActive.current = null;
        void api.preview.cancel(eligibilityRequest.request_id).catch(() => {});
      }
    };
  }, [blockedReason, eligibilityAttempt, revision]);
  ...
  return <section className="mt-5 border-t border-subtle pt-4" aria-label="Settings preview">
    <div className="flex items-center gap-2">
      <button type="button" disabled={busy || !supported} onClick={() => void generate()} className="btn-press rounded-md bg-surface-2 px-3 py-2 text-xs text-fg-secondary disabled:opacity-50">{busy ? "Preparing sample…" : currentEligibility.status === "checking" && !blockedReason ? "Checking preview availability…" : "Preview sample"}</button>
      ...
      {eligibilityError && <button type="button" onClick={() => setEligibilityAttempt(attempt => attempt + 1)} className="rounded-md px-2 py-2 text-xs text-fg-secondary">Retry preview check</button>}
    </div>
    <p className="mt-2 text-xs text-fg-muted">A bounded sample, not an output-size estimate. Originals stay unchanged.</p>
    {unavailableReason && <p role="status" aria-live="polite" className="mt-2 text-xs text-fg-muted">{unavailableReason}</p>}
    {eligibilityError && <p role="alert" className="mt-2 text-xs text-warning">{eligibilityError}</p>}
    ...
    {result && supported && result.source_revision === revision && !details && <div className="mt-3 space-y-3">
      <p className="text-xs text-fg-muted">{result.kind === "video" ? "Muted H.264 viewing sample. Audio, subtitle, and data streams are omitted; source metadata is not preserved. Stream-copy jobs are re-encoded for this preview." : "Sample images omit metadata."}</p>
