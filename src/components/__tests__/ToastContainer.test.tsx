...
describe("ToastContainer live-region ownership", () => {
  ...
  it("mounts a semantic owner for every toast beyond the visual cap", () => {
    ...
    expect(owners.filter((node) => node.classList.contains("sr-only"))).toHaveLength(2);
  });

  it("keeps the toast that owns keyboard focus in the visual set", () => {
    ...
    const focusedOwner = screen.getByRole("status", {
      name: "Finished file 1",
    });
    expect(focusedOwner.classList.contains("sr-only")).toBe(false);
    expect(document.activeElement).toBe(focusedAction);
